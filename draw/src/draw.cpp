...
namespace dw
{
    ...
    */
    ...
    void frame(Matrix &frame, int top, int left, int player)
    {
        if (player == 1)
        {
        ...
        if (player == 2)
        {
            static Matrix buffer2(frame.size(), std::vector<int>(frame[0].size(), -1));
            if (gm::reseting2)
            {
                buffer2 = Matrix(frame.size(), std::vector<int>(frame[0].size(), -1));
            }
            Matrix f(frame.begin(), frame.begin() + 20);
            matrix(f, top, left, &buffer2, "\u30FB");
        }
    }
    void next(std::queue<Tetromino> next5, int top, int left, int player)
    {
        if (player == 1)
        {
        ...
        if (player == 2)
        {
            static Matrix buffer2(15, std::vector<int>(6, -1));
            Matrix next_feild(15, std::vector<int>(6, 0));
            for (int y = 12; next5.size() > 0; y -= 3)
            {
                gm::Piece p(next5.front(), 2, y, 0);
            ...
            matrix(next_feild, top, left, &buffer2);
        }
    }
    void hold(Tetromino &h, int top, int left, int player)
    {
        if (player == 1)
        {
        ...
        if (player == 2)
        {
            static Matrix buffer2(4, std::vector<int>(7, -1));
            Matrix hold_feild(4, std::vector<int>(7, 0));
            if (!h.empty())
            {
                gm::Piece p(h, 3, 1, 0);
                ...
                gm::merge(hold_feild, p);
                matrix(hold_feild, top, left, &buffer2);
            }
        }
    }
